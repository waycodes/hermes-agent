...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "## Durable Obsidian package\n"
    "\n"
    "Every Kanban session must leave a durable package under the configured "
    "Obsidian vault, including code tasks whose source changes remain in a "
    "repository. Run `$HOME/.local/bin/hermes-kanban-vault resolve` for the host-specific root. "
    "The configured defaults are `/Users/varbhar/obsidian_notes_local` on the "
    "Mac and `/home/varbhar/Coding/my-obsidian-notes` on varbhar-gpu. Use the "
    "explicit project name from the card; otherwise derive a human-readable "
    "name from `$HERMES_KANBAN_BOARD`. Run `$HOME/.local/bin/hermes-kanban-vault init "
    "\"<Project Name>\"` before writing.\n"
    "Write only this layout: `Hermes Projects/<Project Name>/references/` "
    "with Python in `_scripts/`, data and images in `_assets/`, supporting "
    "Markdown directly in `references/`, plus `Final Comprehensive "
    "Document.md` and `INDEX.md` at the project root. Never create `t_*`, "
    "task-id, board-slug, or `_kanban/workspaces` directories in the vault. "
    "Use semantic filenames.\n"
    "Supporting workers write their evidence into `references/` and name the "
    "absolute paths in `artifacts`. Orchestrators must create one terminal "
    "synthesis task depending on every evidence lane. That terminal task, or "
    "the worker itself for a single-card board, writes the final document, "
    "reruns `hermes-kanban-vault init` to refresh project and vault indexes, "
    "and must not complete until `$HOME/.local/bin/hermes-kanban-vault validate "
    "\"<Project Name>\"` succeeds.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
